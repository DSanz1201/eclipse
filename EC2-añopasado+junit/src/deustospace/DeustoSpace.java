		...
		try {
			...
			while(sc.hasNextLine()) {
				try {
					...
					Mision nuevaMision = new Mision(campos[0], campos[1], campos[2], Integer.parseInt(campos[5]), Integer.parseInt(campos[4]), Integer.parseInt(campos[3]));
					
					nuevaMision.setNave(new Nave(campos[6], campos[7], Double.parseDouble(campos[8]), Double.parseDouble(campos[9])));
			...
			System.err.println("No se puedo cargar el fichero: " + e.getMessage());
		}
			...
			while(sc.hasNextLine()) {
				...
				if(campos[0].equals("ASTRONAUTA")) {
					...
					for(int i = 3; i < campos.length; i++) {
						habilidades.add(Habilidad.valueOf(campos[i]));
					}
					
					Astronauta nuevo = new Astronauta(campos[1], campos[2], habilidades);
