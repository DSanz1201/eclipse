...
public abstract class Evento implements Revisable {
	...
	public boolean revisar() {
		for(int i = 0; i < partidas.size(); i++) {
			int edad_j1 = partidas.get(i).getBlancas().getEdad();
			int edad_j2 = partidas.get(i).getNegras().getEdad();
			if(edad_j1 < edadMin && edad_j1 > edadMax) {
				return false;
			}
			if(edad_j2 < edadMin && edad_j2 > edadMax) {
				return false;
			}
		}
		return true;
